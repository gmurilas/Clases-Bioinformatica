...
boxplot(data2, col=c("blue", "blue", "blue", "orange", "orange", "orange"), main="GASE5583 - Boxplot")
...
head(wt)
pva1= t.test(prueba1, prueba2)$p.value
pva1
...
abline(v=diff.mean_cutoff,col="blue", iwd=3)
abline(h=-log10(pvalue_cutoff), col="green", iwd=3)
...
pdf("GSE5583 _DE_Heatmap.pdf")
heatmap(filtered, Rowv=rowv, Colv=colv, cexCol=0.7, labRow=FALSE)
dev.off()

# Guardamos los genes diferencialmente expresados y filtrados en un fichero

write.table (filtered, "GSE5583_DE_txt", sep="\t", quote=FALSE)
